...
export class AEMEmbed extends HTMLElement {
  constructor() {
    ...
    // On Edge Delivery this file is served from /scripts/, so the path before
    // it is the code base path (usually ''). When bundled by webpack (this React
    // app) there is no /scripts/ segment, so fall back to '' — otherwise block
    // and style URLs get built from the bundle path and 404.
    const scriptPath = new URL(import.meta.url).pathname;
    window.hlx.codeBasePath = scriptPath.includes('/scripts/')
      ? scriptPath.split('/scripts/')[0]
      : '';
  }
